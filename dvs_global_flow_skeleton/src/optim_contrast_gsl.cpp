    ...
    )
{
  ...
  switch (contrast_measure)
  {
	...
	//std::cout<<"contrast in computeContrast():"<<contrast<<std::endl;
  return contrast;
}
...
double contrast_f_numerical (const gsl_vector *v, void *adata)
{
  ...
  //computeImageOfWarpedEvents(vel, events_subset, img_size, image_warped, optsWarp);
  cv::Mat image_warped;
  computeImageOfWarpedEvents(vel, *poAux_data->poEvents_subset, *poAux_data->img_size, &image_warped, poAux_data->opts->opts_warp_);
  
  contrast = computeContrast(image_warped, poAux_data->opts->contrast_measure_);
	//std::cout<<"contrast in contrast_f_numerical():"<<contrast<<std::endl;
  return -contrast;
}
